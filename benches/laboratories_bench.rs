use aoc2025::laboratories;
use criterion::{criterion_group, criterion_main, Criterion};
use std::hint;

pub fn part_one_benchmark(c: &mut Criterion) {
    let data = include_bytes!("../src/testdata/laboratories/input");
    c.bench_function("Laboratories (Part 1)", |b| {
        b.iter(|| laboratories::part_one(hint::black_box(data)))
    });
}

pub fn part_two_benchmark(c: &mut Criterion) {
    let data = include_bytes!("../src/testdata/laboratories/input");
    c.bench_function("Laboratories (Part 2)", |b| {
        b.iter(|| laboratories::part_two(hint::black_box(data)))
    });
}

pub fn part_two_recursion_benchmark(c: &mut Criterion) {
    let data = include_bytes!("../src/testdata/laboratories/input");
    c.bench_function("Laboratories (Part 2) Recursion)", |b| {
        b.iter(|| laboratories::part_two_recursion(hint::black_box(data)))
    });
}

criterion_group!(
    benches,
    part_one_benchmark,
    part_two_benchmark,
    part_two_recursion_benchmark
);
criterion_main!(benches);
